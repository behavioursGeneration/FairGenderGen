...
from utils.labels import get_other_label, supress_silence_index, get_no_silence_index_from_one_hot, one_hot_to_index
from utils.gradient_reversal.module import GradientReversal

class GenderClassifier(pl.LightningModule):
    def __init__(self):
        super().__init__()
        self.reversal = GradientReversal(alpha=constants.reversal_coeff)
        self.classif_gender_conv1 = Conv(1024, 512, constants.kernel_size)
        self.classif_gender_conv2 = Conv(512, 128, constants.kernel_size)
        self.classif_gender_fc1 = torch.nn.Linear(128 * floor(constants.seq_len/64), 64)
        self.classif_gender_fc2 = torch.nn.Linear(64, 2)

    def forward(self, behaviour_data, lambd):
        #behaviour_data = torch.swapaxes(behaviour_data, 1, 2)
        gender_output = self.reversal(behaviour_data, lambd)
        gender_output = self.classif_gender_conv1(gender_output)
        gender_output = F.max_pool1d(gender_output, kernel_size=2, stride=2)
        gender_output = self.classif_gender_conv2(gender_output)
        #gender_output = F.max_pool1d(gender_output, kernel_size=2, stride=2)
...
class GAN_with_gender(pl.LightningModule):
...
class GAN(pl.LightningModule):
    ...
    def on_train_epoch_start(self):
        ...
        p  = self.current_epoch / constants.number_of_step
        self.lambd = 2. / (1. + np.exp(-10. * p)) - 1
        print(p, self.lambd)
    ...
    def gender_classifier_step(self, latent_representation, gender, lambd):
        gender_index = get_no_silence_index_from_one_hot(gender, "gender")
        targets = latent_representation[gender_index]
        targets_labels = gender[gender_index][:,:2]
        
        gender_output = self.gender_classifier(targets, lambd)
        gender_loss = self.gender_criterion(gender_output, targets_labels)
        _, predicted = torch.max(gender_output, 1)
        predictions = predicted.squeeze()
        all_labels = torch.tensor([one_hot_to_index(ele, "small_gender") for ele in targets_labels]).to(latent_representation)
        accuracy = BinaryAccuracy().to(latent_representation)
        acc = accuracy(predictions, all_labels)
        return gender_loss, acc
    ...
    def training_step(self, batch, batch_idx):
        ...
        g_loss = g_loss + constants.gender_coeff * gender_loss
        g_opt.zero_grad()
        ...
        self.gender_accuracy.append(gender_accuracy)
    ...
    def on_train_epoch_end(self):
        ...
        def _save_losses(dict_loss):
            ...
            metrics = {"device": str(self.device),
                ...
                "gender_loss": dict_loss["avg_gender_loss"].item(),
                "val_gender_loss": dict_loss["avg_val_gender_loss"].item(),
                "gender_accuracy": dict_loss["avg_gender_accuracy"].item(),
                "val_gender_accuracy": dict_loss["avg_val_gender_accuracy"].item(),
        ...
        if((self.current_epoch == 0 or constants.finetune) and not constants.do_resume):
            current_mode = "w"
        ...
        dict_loss["avg_gender_loss"] = torch.stack(self.gender_loss).mean()
        dict_loss["avg_val_gender_loss"] = torch.stack(self.val_gender_loss).mean()
        dict_loss["avg_gender_accuracy"] = torch.stack(self.gender_accuracy).mean()
        dict_loss["avg_val_gender_accuracy"] = torch.stack(self.val_gender_accuracy).mean()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            val_g_loss = val_g_loss + constants.gender_coeff * val_gender_loss

        self.val_gender_loss.append(val_gender_loss)
        self.val_gender_accuracy.append(val_gender_accuracy)
    ...
    def create_loss(self):
        ...
        self.val_loss_au = []
        self.val_gender_loss = []
        self.val_gender_accuracy = []
    ...
    def clear_loss(self):
        ...
        self.loss_au.clear()
        self.gender_loss.clear()
        self.gender_accuracy.clear()
        ...
        self.val_gender_loss.clear()
        self.val_gender_accuracy.clear()
